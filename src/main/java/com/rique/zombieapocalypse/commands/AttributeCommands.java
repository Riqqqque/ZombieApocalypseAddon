...
public final class AttributeCommands {
    ...
    private static final Map<String, ModConfigSpec.BooleanValue> BOOLEAN_SETTINGS = createBooleanSettings();
                                ...
                                .then(Commands.argument("key", StringArgumentType.word())
                                        ...
                                        .then(CommandUtil.toggleArgument("state")
                                                .executes(context -> {
                                                    ...
                                                    ModConfigSpec.BooleanValue setting = BOOLEAN_SETTINGS.get(key);
                                                    if (setting == null) {
                                                        CommandUtil.feedback(context.getSource(),
    ...
    private static int showValue(CommandSourceStack source, String key) {
        ...
        ModConfigSpec.BooleanValue toggle = BOOLEAN_SETTINGS.get(key);
        if (toggle != null) {
    ...
    private static Map<String, ModConfigSpec.BooleanValue> createBooleanSettings() {
        Map<String, ModConfigSpec.BooleanValue> map = new LinkedHashMap<>();
    ...
    private static void addMultiplier(Map<String, NumericSetting> map, String prefix, ModConfigSpec.DoubleValue multiplier) {
        map.put(prefix + ".multiplier", doubleSetting(multiplier, 0.0, 10.0));
    }

    private static NumericSetting doubleSetting(ModConfigSpec.DoubleValue value, double minimum, double maximum) {
        return new NumericSetting(false, minimum, maximum, value::get, newValue -> Config.set(value, newValue));
    }

    private static NumericSetting intSetting(ModConfigSpec.IntValue value, int minimum, int maximum) {
